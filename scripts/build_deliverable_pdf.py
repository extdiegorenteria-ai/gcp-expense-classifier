...
<body>
  ...
  <h2>5. Infraestructura como Código Modular (Terraform)</h2>
  <p>
    Toda la infraestructura se diseñó siguiendo el estándar de producción mediante separación modular de responsabilidades:
  </p>
  <ul>
    <li><code>main.tf</code> & <code>services.tf</code>: Providers y activación de APIs de GCP.</li>
    <li><code>storage.tf</code> & <code>bigquery.tf</code>: Landing zone en GCS y Data Warehouse particionado.</li>
    <li><code>iam.tf</code>: Principio de menor privilegio para la Service Account y Eventarc.</li>
    <li><code>function.tf</code>: Definición serverless de Cloud Functions v2 y Eventarc.</li>
    <li><code>variables.tf</code> & <code>outputs.tf</code>: Parametrización y exportación de endpoints.</li>
  </ul>
  ...
  <ul>
    ...
    <li><strong>Inferencia:</strong> Gemini 2.5 Flash en Vertex AI tiene un coste inferior a $0.05 USD por cada 500 comprobantes procesados, cubierto por los créditos iniciales de Google Cloud.</li>
  </ul>

  <h2>7. Evidencia de Ejecución en Producción (Prueba E2E)</h2>
  <p>
    Se ejecutó una prueba de extremo a extremo subiendo el comprobante <code>ticket_restaurante_001.jpg</code> a Cloud Storage. El flujo se activó reactivamente mediante Eventarc y persistió exitosamente en BigQuery:
  </p>
  <table>
    <thead>
      <tr>
        <th>Atributo</th>
        <th>Valor Extraído por el Modelo y Almacenado en BigQuery</th>
      </tr>
    </thead>
    <tbody>
      <tr>
        <td><strong>Fecha de Gasto</strong></td>
        <td><code>2026-09-18</code></td>
      </tr>
      <tr>
        <td><strong>Comercio</strong></td>
        <td><code>RESTAURANTE EL BUEN SABOR</code></td>
      </tr>
      <tr>
        <td><strong>Categoría Asignada</strong></td>
      ...
      <tr>
        <td><strong>Monto Total Facturado</strong></td>
        <td><code>S/ 45.00</code></td>
      </tr>
      <tr>
        <td><strong>Impuesto Desglosado (IGV 18%)</strong></td>
        <td><code>S/ 6.86</code></td>
      </tr>
      <tr>
        <td><strong>Confianza del Modelo</strong></td>
        <td><code>0.95</code> (95%)</td>
      </tr>
      <tr>
        <td><strong>URI de Auditoría en GCS</strong></td>
        <td><code>gs://gcp-expense-darv-6849-gastos-raw/inbox/ticket_restaurante_001.jpg</code></td>
      </tr>
    </tbody>
  </table>
